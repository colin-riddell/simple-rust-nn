...
//  let mut network = Network::new(vec![
//         Neuron::new(vec![random_f64(), random_f64(), random_f64()], 0.0),
//     ]);
...
// THIS WORKS REALLY WELL FOR SIMPLE PROBLEM:
// use std::io::Write;
// use rand::*;
// use std::fs::*;
// use image::*;
...
// impl Network {
//     fn new(input_size: usize, output_size: usize) -> Network {
...
//         for _ in 0..epochs {
//             for (input, output) in inputs.iter().zip(outputs.iter()) {
//                 let mut preds = self.predict(input);
//                 for (pred, target) in preds.iter_mut().zip(output.iter()) {
//                     *pred = *pred * (1.0 - *pred) * (*target - *pred);
//                 }

//                 for (neuron, pred) in self.neurons.iter_mut().zip(preds.iter()) {
//                     for (weight, input) in neuron.weights.iter_mut().zip(input.iter()) {
//                         *weight += rate * *input * *pred;
//                     }
//                     neuron.bias += rate * *pred;
//                 }
//             }
//         }
//     }
